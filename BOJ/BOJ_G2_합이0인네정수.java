...
public class BOJ_G2_합이0인네정수 {
	...
	// 2차: RGB 거리같은건가?
	static int getLowerPos(int[] arr, int V) {
	...
	static int getUpperPos(int[] arr, int V) {
		int left = 0, right = arr.length;
		while (left < right) {
			int mid = left + (right - left) / 2;
			// V보다 작거나 같을 때
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		while (arr1Right <= n*n) {
			...
			int arr1Left = getLowerPos(arr1, arr1[arr1Pos]);
			arr1Right = getUpperPos(arr1, arr1[arr1Pos]);
			int arr1Size = arr1Right - arr1Left;
			int arr2Right = getUpperPos(arr2, -arr1[arr1Pos]);
			int arr2Left = getLowerPos(arr2, -arr1[arr1Pos]);
